...
# IAM Role for Task Execution (Pulling images, putting logs)
...
# IAM Role for ECS Task (Running backend/frontend operations)
...
# CloudWatch Log Group for Application Container Logging
...
resource "aws_lb_listener" "http" {
  load_balancer_arn = aws_lb.external.load_balancer_arn
  port              = "80"
  protocol          = "HTTP"

  # Default action redirects to frontend target group
  default_action {
    type             = "forward"
...
# ALB Listener Rule for routing API endpoints to backend target group
...
# ECS Task Definition for React Frontend
